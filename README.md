...
| # | Module          | What it does                                                                  |
|---|-----------------|-------------------------------------------------------------------------------|
| 1 | **File Viewer** | Prompts for a filename, opens it, and prints its contents to the screen.      |
| 2 | **Converter**   | Reads a decimal number (0–65535) and prints it in **hexadecimal** and **binary** (4-bit grouped). |
| 3 | **Clock**       | Reads the current system time via `int 21h / 2Ch` and prints it as `HH:MM:SS`.|
| 4 | **Encryption**  | Takes a message + numeric key, performs an **XOR cipher**, prints the encrypted hex bytes, then decrypts back to the original. |
| 5 | **Exit**        | Returns control to DOS.                                                       |
...
├── converter.asm    Module 2 - decimal -> hex / binary (_converter)
...
- Based on the key (`'1'` … `'5'`), it `call`s the appropriate module's exported symbol.
...
- **NASM** (Netwide Assembler) — generates `obj` files
  Download: <https://www.nasm.us/>
- **ALINK** — small 16-bit linker that produces DOS / PE executables
  Download: <http://alink.sourceforge.net/>
- **DOSBox** (optional, recommended for running) — needed on modern Windows because 64-bit Windows cannot natively run 16-bit DOS executables
...
- The number converter accepts 0–65535 (16-bit unsigned). Larger inputs will overflow.
